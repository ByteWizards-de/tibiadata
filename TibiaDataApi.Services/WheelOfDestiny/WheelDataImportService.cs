...
namespace TibiaDataApi.Services.WheelOfDestiny
{
        ...
        IGemModDataImportService gemModDataImportService) : IWheelDataImportService
    {
        ...
        private const string PlannerSourceTitle = "Wheel of Destiny Planner";
        private const string PlannerSourceUrl = "https://www.tibia.com/community/?subtopic=wheelofdestinyplanner";
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await executionStrategy.ExecuteAsync(async () =>
            {
                ...
                List<WheelPerk> removedPerks = existing
                                               .Where(entry => !incomingKeys.Contains(entry.Key) && !IsPlannerBackfilled(entry))
                                               .ToList();
            ...
            CancellationToken cancellationToken)
        {
            ...
            await BackfillMissingPlannerPerksAsync(db, layoutSnapshot, perks, cancellationToken);
            ...
            logger.LogInformation(
                ...
                private async Task BackfillMissingPlannerPerksAsync(
            TibiaDbContext db,
            WheelPlannerLayoutSnapshot layoutSnapshot,
            List<WheelPerk> perks,
            CancellationToken cancellationToken)
        {
            Dictionary<string, WheelPerk> lookup = new(StringComparer.Ordinal);
            foreach(WheelPerk perk in perks)
            {
                lookup.TryAdd(BuildPerkLookupKey(perk.Vocation, perk.Type, perk.Name), perk);
            }

            bool IsResolvable(WheelVocation vocation, WheelPerkType type, string name) =>
                ExpandPerkLookupCandidates(name)
                    .Any(candidate => lookup.ContainsKey(BuildPerkLookupKey(vocation, type, candidate)));

            DateTime now = DateTime.UtcNow;
            int created = 0;

            foreach(WheelPlannerSectionSnapshot section in layoutSnapshot.Sections)
            {
                cancellationToken.ThrowIfCancellationRequested();

                string convictionName = ExtractConvictionPerkName(section.ConvictionText);
                if(!IsResolvable(section.Vocation, WheelPerkType.Conviction, convictionName))
                {
                    AddBackfilledPerk(CreatePlannerConvictionDraft(section, convictionName), now, db, perks, lookup);
                    created++;
                }

                foreach(string dedicationName in ExtractDedicationPerkNames(section.DedicationText))
                {
                    if(IsResolvable(section.Vocation, WheelPerkType.Dedication, dedicationName))
                    {
                        continue;
                    }

                    AddBackfilledPerk(CreatePlannerDedicationDraft(section, dedicationName), now, db, perks, lookup);
                    created++;
                }
            }

            foreach(WheelPlannerRevelationSlotSnapshot slot in layoutSnapshot.RevelationSlots)
            {
                cancellationToken.ThrowIfCancellationRequested();

                string revelationName = NormalizePlannerRevelationPerkName(slot.PerkName);
                if(IsResolvable(slot.Vocation, WheelPerkType.Revelation, revelationName))
                {
                    continue;
                }

                AddBackfilledPerk(CreatePlannerRevelationDraft(slot, revelationName), now, db, perks, lookup);
                created++;
            }
            ...
            if(created > 0)
            {
                await db.SaveChangesAsync(cancellationToken);
                logger.LogInformation(
                    "Backfilled {Count} wheel perk(s) from the official planner that were missing from the wiki catalog.",
                    created);
            }
        }

        private static void AddBackfilledPerk(
            WheelPerkDraft draft,
            DateTime timestampUtc,
            TibiaDbContext db,
            List<WheelPerk> perks,
            Dictionary<string, WheelPerk> lookup)
        {
            WheelPerk entity = CreateEntity(draft, timestampUtc);
            db.WheelPerks.Add(entity);
            perks.Add(entity);
            lookup.TryAdd(BuildPerkLookupKey(entity.Vocation, entity.Type, entity.Name), entity);
        }

        private static WheelPerkDraft CreatePlannerConvictionDraft(
            WheelPlannerSectionSnapshot section,
            string convictionName)
        {
            bool isAugmentation = GetFirstNonEmptyLine(section.ConvictionText)
                .StartsWith("Augmented ", StringComparison.OrdinalIgnoreCase);

            if(isAugmentation)
            {
                IReadOnlyList<WheelPerkStageDraft> stages = ParsePlannerAugmentationStages(section.ConvictionText);
                string augmentationDescription = $"Conviction augmentation for {convictionName}.";
                ...
                return CreatePerkDraft(
                    ...
                    new
                    {
                        SourceKind = "official-planner-augmentation",
                        SourceSectionKey = section.SectionKey,
                        IsAugmentation = true,
                        BackfilledFromPlanner = true
                    },
                    [
                        new WheelPerkOccurrenceDraft(null, 1, GetConvictionOccurrenceRequiredPoints(1), false, "First occurrence unlocks stage 1."),
                        new WheelPerkOccurrenceDraft(null, 2, GetConvictionOccurrenceRequiredPoints(2), false, "Second occurrence unlocks stage 2.")
                    ],
                    stages);
            }

            string description = NormalizeOptionalText(CleanWikiText(section.ConvictionText)) ?? convictionName;
            ...
            return CreatePerkDraft(
                ...
                new
                {
                    SourceKind = "official-planner-conviction",
                    SourceSectionKey = section.SectionKey,
                    BackfilledFromPlanner = true
                },
                [
                    new WheelPerkOccurrenceDraft(null, 1, GetConvictionOccurrenceRequiredPoints(1), true, "Backfilled from the official planner.")
                ],
                []);
        }

        private static WheelPerkDraft CreatePlannerDedicationDraft(
            WheelPlannerSectionSnapshot section,
            string dedicationName)
        {
            string description = $"Dedication perk for {dedicationName}.";
            ...
            return CreatePerkDraft(
                ...
                new
                {
                    SourceKind = "official-planner-dedication",
                    SourceSectionKey = section.SectionKey,
                    BackfilledFromPlanner = true
                },
                [
                    new WheelPerkOccurrenceDraft(null, 1, DedicationOccurrenceRequiredPoints, true, "Backfilled from the official planner.")
                ],
                []);
        }

        private static WheelPerkDraft CreatePlannerRevelationDraft(
            WheelPlannerRevelationSlotSnapshot slot,
            string revelationName)
        {
            string description =
                $"Revelation perk for {revelationName}. Stage details are enriched from the wiki when available.";
            ...
            return CreatePerkDraft(
                ...
                new
                {
                    SourceKind = "official-planner-revelation",
                    SourceSlotKey = slot.SlotKey,
                    BackfilledFromPlanner = true
                },
                [
                    new WheelPerkOccurrenceDraft(null, 1, RevelationMinimumDomainPoints, false, "Revelation perks start unlocking at 250 domain points.")
                ],
                []);
        }

                private static IReadOnlyList<WheelPerkStageDraft> ParsePlannerAugmentationStages(string convictionText)
        {
            List<string> effects = convictionText
                                   .Split('\n')
            ...
            {
                byte stage = (byte)(index + 1);
                string effect = effects[index];

                stages.Add(new WheelPerkStageDraft(
                    stage,
                    WheelStageUnlockKind.OccurrenceCount,
                    stage,
                    Truncate(effect, 2000),
                    BuildStageDetailsJson([new KeyValuePair<string, string>("effect", effect)]),
                    (short)stage));
            }
        ...
        private static bool IsPlannerBackfilled(WheelPerk perk) =>
            perk.MetadataJson is { Length: > 0 } metadata &&
            metadata.Contains("\"BackfilledFromPlanner\":true", StringComparison.OrdinalIgnoreCase);
