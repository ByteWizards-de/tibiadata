...
namespace TibiaDataApi.Services.WheelOfDestiny
{
        ...
        ILogger<OfficialWheelPlannerLayoutSource> logger) : IWheelPlannerLayoutSource
    {
        ...
        private static string ResolveBrowserExecutablePath()
        {
            ...
            string? match = EnumerateBrowserCandidates().FirstOrDefault(File.Exists);
        ...
        private static IEnumerable<string> EnumerateBrowserCandidates()
        {
            ...
            string dataHome = Environment.GetEnvironmentVariable("XDG_DATA_HOME") is { Length: > 0 } xdgDataHome
                ? xdgDataHome
                : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "share");

            string userFlatpakRoot = Path.Combine(dataHome, "flatpak");

            foreach (string flatpakRoot in new[] { userFlatpakRoot, "/var/lib/flatpak" })
            {
                yield return Path.Combine(
                    flatpakRoot, "app", "com.google.Chrome", "x86_64", "stable", "active", "files", "extra", "chrome");
                yield return Path.Combine(
                    flatpakRoot, "app", "org.chromium.Chromium", "x86_64", "stable", "active", "files", "extra", "chromium");
            }
        }
