...
### Changed
- Wheel of Destiny data now reflects in-game balance updates immediately: perks that appear in the official planner before the TibiaWiki catalog is updated (e.g. renamed or newly added perks from the 2026 Vocation Adjustments) are imported directly from the planner, with conviction augmentation tier effects captured as stages, and are automatically enriched from the wiki once it catches up.
- Updated NuGet dependencies to their latest patch releases.

### Fixed
- Fixed the Wheel of Destiny import aborting when the official planner referenced perks that were not yet present in the TibiaWiki catalog.
