...
namespace TibiaDataApi.Services.Tests
{
    public sealed class WheelDataImportServiceTests
    {
        ...
        [Fact]
        public async Task ImportAsync_BackfillsPlannerPerksMissingFromWikiCatalog()
        {
            await using TibiaDbContext db = CreateDbContext();
            ...
            WheelPerk shieldSlam = Assert.Single(
                perks,
                entry => entry.Name == "Shield Slam" &&
                         entry.Vocation == WheelVocation.EliteKnight &&
                         entry.Type == WheelPerkType.Conviction);
            Assert.Equal(2, shieldSlam.Stages.Count);
            Assert.Equal(WheelStageUnlockKind.OccurrenceCount, shieldSlam.Stages[0].UnlockKind);
            Assert.Contains("Base Healing", shieldSlam.Stages[0].EffectSummary ?? string.Empty);
            Assert.Contains("Cooldown", shieldSlam.Stages[1].EffectSummary ?? string.Empty);
            Assert.Equal(2, shieldSlam.Occurrences.Count);

            
            WheelPerk newRevelation = Assert.Single(
                perks,
                entry => entry.Name == "Brand New Revelation" &&
                         entry.Type == WheelPerkType.Revelation);
            Assert.Empty(newRevelation.Stages);

            
            WheelSection section = Assert.Single(
                await db.WheelSections.Include(entry => entry.ConvictionWheelPerk).ToListAsync());
            Assert.Equal("Shield Slam", section.ConvictionWheelPerk.Name);

            
            await service.ImportAsync(db);
            Assert.Single(
                await db.WheelPerks
                        .Where(entry => entry.Name == "Shield Slam" && entry.Vocation == WheelVocation.EliteKnight)
                        .ToListAsync());
        }
